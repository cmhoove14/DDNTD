...
Human infection across treatment groups, $j\in(\underline{\textbf{T}}reated,\underline{\textbf{U}}ntreated)$ is modeled via state variables $W_{j}$ representing the mean worm burden in each human population group, assumed to be negative binomially distributed with independent clumping parameters $\kappa_{j}$.

$$\frac{dW_{j}}{dt}=\lambda-(\mu_W+\mu_H)W_{j}$$

Human infection dynamics are linked to the intermediate host snail population via the snail-to-man FOI or "worm acquisition rate", $\lambda$. This process is modeled as the product of cercarial density, $C$, exposure/contamination rate, $\omega$, and the probability of a cercarial contact resulting in an adult worm infection, $\alpha$. Infected snails shed cercariae at daily rate $\theta$, giving $C=\theta I$ and $\lambda=\alpha\omega\theta I$.
...
Miracidial density is estimated as the product of mean egg output in each worm burden group, $\mathcal{E}_{j}$; the exposure/contamination rate , $\omega$; schistosome egg viability, $v$, and the treatment coverage, $\mathcal{T}$. 

Mean egg output is estimated assuming a 1:1 sex ratio and a negative binomial distribution of adult worms among definitive human hosts in each age and treatment strata. Within each of these strata, key density dependent functions representing the mating probability, $\Phi(W_{j})$, and reductions in egg production due to crowding, $\rho(W_{j})$, are estimated as a function of the mean worm burden, $W_{j}$, and the dispersion parameter of the negative binomial distribution, $\kappa_{j}$ (details in SI;  `r citet("10.1016/0025-5564(77)90030-X")`). Previous analyses of the distribution of estimated worm counts within definitive human host populations have shown that the dispersion parameter varies predictably as a function of the overall mean worm burden and can change quite dramatically following reductions in the worm burden such as those induced by MDA. `r citet(c("10.1016/S1473-3099(16)30073-1", "10.1371/journal.pone.0115875"))` In particular, $\kappa_{j}$ decreases, implying more skewed distributions in which fewer individuals harbor more worms, as $W_{j}$ decreases. This leads to an increase in the mating probability, $\Phi$, relative to an assumption of constant values of $\kappa_{j}$, even as worm populations decrease due to MDA or other interventions. The dispersion parameter in each strata is therefore modeled as a function of the mean worm burden:

$$\kappa_{j}=e^{\big(a-b\log(W_{j})\big)}$$

and both $\Phi$ and $\rho$ can be expressed in terms of the mean worm burden alone. 

The mean egg output from each strata can therefore be estimated as $\mathcal{E}_{j}=0.5W_{j}\Phi(W_{j})\rho(W_{j})mU$ with additional model parameters defined in Table 1. Taking a weighted sum across treatment groups and multiplying by the total human population, $\mathbf{H}$, gives an estimate of total daily miracidial density:

$$M=0.5\mathbf{H}mvU\omega\big(\mathcal{T}W_{T}\Phi(W_{T})\rho(W_{T})+(1-\mathcal{T})W_{U}\Phi(W_{U})\rho(W_{U)\big)$$
```{r echo=FALSE}
...
```{r lhs, include = FALSE, message = FALSE, fig.width=8, fig.height=8}
...
#reorder parameters in order of LHC indices  
  ...
  lhcpars_W_bp <- apply(lhcpars[1:10,], 1, 
                        FUN = function(x){use_pars <- as.numeric(x) ; names(use_pars) <- names(fit_pars) ; W_bp(use_pars)}) 
  
  W_bp_pcc <- pcc(X = lhcpars[!is.na(lhcpars_W_bp),],
                  y = lhcpars_W_bp[!is.na(lhcpars_W_bp)],
                  rank = TRUE)
  
  W_bp_pcc_df <- data.frame(vars = names(fit_pars),
                            PRCC = W_bp_pcc$PRCC[[1]])

W_bp_pcc_df %>%   
  ggplot(aes(x = vars, y = PRCC)) +
    theme_bw()+
    #scale_y_continuous(limits = c(-1,1), breaks = c(-1,0,1))+
    geom_bar(fill = 'black', stat = 'identity', width = 0.25)+
    labs(title = expression(W[bp]~Sensitivity), x = 'Parameter', y = 'Mean PRCC')#+geom_errorbar(aes(x = vars, ymin = (mean - sd), ymax = (mean + sd)), width = 0.1)
...
```{r include = FALSE, message = FALSE, warning = FALSE, cache = TRUE, fig.width=8, fig.height=8}
cvrg_df <- expand.grid(W_t = exp_seq(0.1,100,50),
                       W_u = exp_seq(0.1,100,50),
                       K = seq(fit_pars["K"], fit_pars["K"]*0.7, length.out = 10),
                       omega = seq(fit_pars["omega"], fit_pars["omega"]*0.7, length.out = 10),
                       epsilon = 0.94) %>% 
...
cvrg_df %>% 
  mutate(case_when(cvrg > 0 & cvrg < 100 ~ cvrg,
                   cvrg < 0 ~ NA_real_,
                   cvrg > 100 ~ NA_real_)) %>% 
