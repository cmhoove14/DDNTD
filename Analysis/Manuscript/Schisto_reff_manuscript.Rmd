...
Together, this evidence suggests a mismatch between the theoretical underpinnings of schistosomiasis control and elimination and current strategies to achieve these goals. Here, we attempt to elaborate on this discrepancy by documenting sources of resilience to schistosomiasis control and elimination by MDA that may explain rapid rebound and persistent hot-spot phenomena. Building on our recently published work `r citet("10.1371/journal.pntd.0006794")` that examines the influence of PDD on schistoosmiasis transmission dynamics, we use the bounce back rate, $BBR$, to quantify the rate of rebound following MDA. These sources of resilience are incorporated into a dynamic model of schistosomiasis transmission. Their effects on transmission during regular MDA are quantified via their influence on the effective reproduction number $R_{eff}(W)$. From this model, we derive an analytic expression of the transmission breakpoint, $W_{bp}$, and identify its main determinants. We conclude with simulations of a stochastic version of the schistosomiasis model to determine the probability of elimination across a range of transmission and intervention scenarios.
...
We assume infected snails, $I$, do not reproduce and the snail population growth rate, $f_N$, is logistic with max reproduction rate, $r$, and carrying capacity, $K$, giving $f_N=r(1-N/K)$. The snail and mean worm burden compartments of the model are linked by snail-to-man force of infection (FOI), $\lambda$, and man-to-snail FOI, $\Lambda$, each of which is described further below. 
...
$$\Lambda=\Lambda_0\big(1-e^{- M/N}\big)$$

where the miracidium invasion rate, $\Lambda_0$, is moderated by the probability of invasion assuming a Poisson distribution of miracidia per snail (see `r citet("10.1371/journal.pntd.0006514")`). Miracidial density is estimated as the product of mean egg output in each worm burden group, $\mathcal{E}_{ij}$; a contamination coefficient related to differences in sanitation and other behaviors between SAC and adults, $\omega_i$; schistosome egg viability, $v$, and the population fraction of each group, $h_{ij}$. 

Mean egg output is estimated assuming a sex ratio of 1 and a single negative binomial distribution in each segment of the human population describing the distribution of adult worms. This leads to an estimate of the mean number of mated adult female worms per host, $W_{f,ij}=0.5W_{ij}\Phi(W_{ij})$, with mating probability, $\Phi(W_{ij})$ estimated as in `r citet("10.1016/0025-5564(77)90030-X")`. Mated female worms lay eggs at rate $m$, measured in eggs per 10mL urine, with viability, $v$, which combined with density dependent fecundity, $\rho(W_{ij})$, and the mean volume of urine per individual, $U_{ij}$, gives the product $W_{f,ij}m\rho(W_{ij})U_{ij}$ as an estimate of the mean miracidia produced per person in each age and treatment group. Taking a weighted sum across all groups and multiplying by the total human population, $mathbf{H}$, gives an estimate of total miracidial density:

$$M=\mathbf{H}\sum_i\sum_j 0.5W_{ij}h_{ij}\Phi(W_{ij})m\rho(W_{ij})U_{ij}\omega$$
