...
Reffs %>% 
  ...
  ggplot(aes(x = test_ws, y = Reff, lty = Clumping_Function)) +
    ...
    labs(x = expression(Mean~Worm~Burden~(italic(W))),
         y = expression(italic(R[eff]))) #+ theme(legend.position = "none")
```

### See how $R_eff$ changes over course of stochastic simulation    

```{r stoch_reff_time}
schisto_stoch_sim_reff <- schisto_stoch_sim %>% 
  mutate(Reff = pmap_dbl(list(parameters = list(base_pars),
                              W = W,
                              kap = base_pars["k"]), getReff))
