...
rho_k0.1 <- sapply(test_ws, rho_Wk, zeta = 5e-3, k = 0.1)

as.data.frame(cbind(test_ws,
                    rho_k0.1)) %>% 
  ggplot(aes(x = test_ws, y = rho_k0.1)) +
    geom_line(size = 1.2) +
    theme_classic() +
    theme(title = element_text(size = 18),
          axis.title = element_text(size = 15),
...
gamma_k0.1 <- sapply(test_ws, gam_Wv, xi = 2.8e-2)
...
Reffs <- data.frame(test_ws = test_ws) %>% 
  mutate(kap = sapply(test_ws, k_from_log_W),
         Absent = pmap_dbl(list(parameters = list(base_pars),
                                     W = test_ws,
                                     kap = kap), getReff_noPDD),
