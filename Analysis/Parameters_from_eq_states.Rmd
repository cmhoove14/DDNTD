...
`r latexImg("R_{eff}(W)=\\frac{K\\alpha\\omega\\theta\\sigma\\big(1+\\frac{\\Lambda(W)}{\\mu_N+\\sigma}-\\frac{\\mu_I}{r}-\\frac{\\Lambda(W)}{r}\\big)}{W\\big(\\mu_W+\\mu_H\\big)\\Big(\\frac{\\mu_I(\\mu_N+\\sigma)}{\\Lambda(W)}+\\mu_I+\\sigma\\Big)\\Big(1+\\frac{\\Lambda(W)}{\\mu_N+\\sigma}\\Big)}")`
...
`r latexImg("cvrg=\\frac{\\frac{W_{bp}}{W_t}-1}{-\\epsilon}")`
